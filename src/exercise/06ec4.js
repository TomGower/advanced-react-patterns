...
function useOnChangeReadOnlyWarning(
  controlPropValue, controlPropName,
  componentName,
  hasOnChange,
...
function useToggle({
  ...
  if (process.env.NODE_ENV !== 'production') {
    // KCD note: this will never change for the lifetime of the application, regardless of rendering
    // OTHERWISE DO NOT VIOLATE THE RULES OF HOOKS

    // eslint-disable-next-line react-hooks/rules-of-hooks
    useControlledSwitchWarning(controlledOn, 'on', 'useToggle')
    // eslint-disable-next-line react-hooks/rules-of-hooks
    useOnChangeReadOnlyWarning(controlledOn, 'on', 'useToggle', hasOnChange, readOnly, 'readOnly', 'initialOn', 'onChange')
  }

  const dispatchWithOnChange = (action) => {
    if (!onIsControlled) dispatch(action)
    const nextState = reducer({...state, on}, action)
    if (hasOnChange) onChange(nextState, action)
  }
