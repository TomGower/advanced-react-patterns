...
function useToggle({
  ...
  React.useEffect(() => {
    warning(
      ...
      `An \`on\` prop was provided to useToggle without an \`onChange\` handler. This will produce a read-only toggle. If you want it to not be read-only, use \`initialOn\`. Otherwise, set either \`onChange\` or \`readOnly\`.`
    )
  }, [hasOnChange, onIsControlled, readOnly])
